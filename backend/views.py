...
def _require_admin(request):
    """Returns HttpResponseForbidden if the user isn't an admin, else None."""
    u = request.user
    if not u.is_authenticated:
        return HttpResponseForbidden('Login required')
    if not (u.is_staff or getattr(u, 'user_type', None) == 'admin'):
        return HttpResponseForbidden('Admin access required')
    return None


# ── Seeder (runs on every visit to index; safe & idempotent) ──
SECTION_TO_TAB = {
    'work_types':           'operations',
    'accreditations':       'operations',
    'operating_areas':      'operations',
    'vehicle_types':        'fleet',
    'special_cargo':        'fleet',
    'nhvr_configurations':  'fleet',
    'safety_policies':      'risk',
    'compliance_practices': 'subcontractors',
    'reporting_process':    'incidents',
}
SECTIONS_WITH_CATEGORIES = {'nhvr_configurations'}

def _ensure_seed_data():
    """Create predefined Tab and Section rows if they don't exist yet."""
    for i, (key, label) in enumerate(Tab.TAB_CHOICES):
        Tab.objects.get_or_create(key=key, defaults={'title': label, 'order': i})
    for i, (key, label) in enumerate(Section.SECTION_CHOICES):
        tab_key = SECTION_TO_TAB.get(key)
        if not tab_key:
            continue
        try:
            tab = Tab.objects.get(key=tab_key)
        ...
        Section.objects.get_or_create(
            ...
            defaults={
                ...
                'title': label,
                'order': i,
                'has_categories': key in SECTIONS_WITH_CATEGORIES,
            },
        )
...
def form_builder_index(request):
    ...
    tabs = list(Tab.objects.prefetch_related('sections').order_by('order'))
    for tab in tabs:
        for s in tab.sections.all():
            s.option_count = s.options.count()
            s.category_count = s.categories.count() if s.has_categories else 0

    return render(request, 'backend/dashboard/form_builder/index.html', {'tabs': tabs})


@login_required
def form_builder_section(request, section_key):
    forbidden = _require_admin(request)
    if forbidden:
        return forbidden

    section = get_object_or_404(Section, key=section_key)
    context = {'section': section}

    if section.has_categories:
        context['categories'] = section.categories.prefetch_related('options').order_by('order')
    else:
        context['options'] = section.options.order_by('order')

    return render(request, 'backend/dashboard/form_builder/section.html', context)
...
def api_update_tab(request, pk):
    ...
    try:
        ...
        tab.title = title
        tab.description = (data.get('description') or '').strip()
        tab.save()
...
def api_create_category(request):
    ...
    try:
        ...
        if not section.has_categories:
            return JsonResponse({'success': False, 'error': 'This section does not use categories'}, status=400)
        name = (data.get('name') or '').strip()
        if not name:
            return JsonResponse({'success': False, 'error': 'Name is required'}, status=400)
        cat = CheckboxCategory.objects.create(
            section=section,
            name=name,
...
def api_update_category(request, pk):
    ...
    if forbidden:
        return forbidden
    try:
        cat = get_object_or_404(CheckboxCategory, pk=pk)
        data = _json_body(request)
        name = (data.get('name') or '').strip()
        if not name:
            return JsonResponse({'success': False, 'error': 'Name is required'}, status=400)
...
def api_delete_category(request, pk):
    ...
    if forbidden:
        return forbidden
    try:
        get_object_or_404(CheckboxCategory, pk=pk).delete()
        return JsonResponse({'success': True})
    except Exception as e:
        return JsonResponse({'success': False, 'error': str(e)}, status=400)
...
# ── Option ──
def _save_option_fields(opt, data, section):
    opt.label = (data.get('label') or '').strip()
    opt.value = (data.get('value') or '').strip()
    opt.description = (data.get('description') or '').strip()
    opt.order = int(data.get('order') or 0)
    opt.is_active = bool(data.get('is_active', True))
    opt.requires_date = bool(data.get('requires_date', False))

    cat_id = data.get('category_id')
    if section.has_categories:
        if not cat_id:
            raise ValidationError('Category is required for this section')
        opt.category = get_object_or_404(CheckboxCategory, pk=cat_id, section=section)
    else:
        opt.category = None
...
def api_create_option(request):
    ...
    try:
        ...
        opt = CheckboxOption(section=section)
        _save_option_fields(opt, data, section)
        opt.full_clean()       # runs the model's clean()
        opt.save()
        return JsonResponse({'success': True, 'id': opt.id})
    except IntegrityError:
        return JsonResponse({'success': False, 'error': 'A value with this name already exists in this section'}, status=400)
...
@require_POST
@login_required
def api_update_option(request, pk):
    forbidden = _require_admin(request)
    if forbidden:
        return forbidden
    try:
        opt = get_object_or_404(CheckboxOption, pk=pk)
        data = _json_body(request)
        _save_option_fields(opt, data, opt.section)
        opt.full_clean()
        opt.save()
        return JsonResponse({'success': True})
    except IntegrityError:
...
@require_POST
@login_required
def api_delete_option(request, pk):
    forbidden = _require_admin(request)
    if forbidden:
        return forbidden
    try:
        get_object_or_404(CheckboxOption, pk=pk).delete()
        return JsonResponse({'success': True})
    except Exception as e:
        return JsonResponse({'success': False, 'error': str(e)}, status=400)