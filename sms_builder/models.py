...
# ══════════════════════════════════════════════════════════════
# DYNAMIC CHECKBOX SYSTEM (admin-managed)
# ══════════════════════════════════════════════════════════════

class Tab(models.Model):
    """Top-level form tabs. Keys are fixed; admin only edits title/description (used in PDF)."""
    TAB_CHOICES = [
    ...
    key         = models.CharField(max_length=50, choices=TAB_CHOICES, unique=True)
...
class Section(models.Model):
    """Section labels within a tab. Keys are fixed; admin only edits title/description."""
    SECTION_CHOICES = [
        # Operations tab
        ('work_types',           'Work Types'),
        ('accreditations',       'Accreditation'),
        ('operating_areas',      'Operating Area'),
        # Fleet tab
        ('vehicle_types',        'Vehicle Types'),
        ('special_cargo',        'Special Cargo'),
        ('nhvr_configurations',  'NHVR Vehicle Configurations'),
        # Risk tab
        ('safety_policies',      'Safety Management Policies'),
        # Subcontractors tab
        ('compliance_practices', 'Subcontractor Compliance Practices'),
        # Incidents tab
        ('reporting_process',    'Incident Reporting Process'),
    ]
    tab            = models.ForeignKey(Tab, on_delete=models.CASCADE, related_name='sections')
...
class CheckboxCategory(models.Model):
    """Accordion groupings inside a section (e.g. '1. Common Rigid Trucks – General Access').
    Fully admin-managed: add/edit/remove."""
...
class CheckboxOption(models.Model):
    """Individual checkbox option. Fully admin-managed."""
    section       = models.ForeignKey(Section, on_delete=models.CASCADE, related_name='options')
    category      = models.ForeignKey(
        CheckboxCategory, on_delete=models.CASCADE,
        related_name='options', null=True, blank=True,
        help_text="Required only if the section uses categories (e.g. NHVR)"
    )
    label         = models.CharField(max_length=255, help_text="Short label shown next to the checkbox")
    value         = models.CharField(max_length=255, help_text="Stable identifier; keep short, no spaces preferred")
    description   = models.TextField(blank=True, help_text="Long description used when generating the PDF")
    # Optional: lets a checkbox trigger a related date field (handles None/TruckSafe/WAHVA today)
    ...
    def clean(self):
        from django.core.exceptions import ValidationError
        if self.section_id and self.section.has_categories and not self.category_id:
            raise ValidationError("This section uses categories — a category is required.")
        if self.section_id and not self.section.has_categories and self.category_id:
            raise ValidationError("This section does not use categories — leave category blank.")
        if self.category_id and self.category.section_id != self.section_id:
            raise ValidationError("Category must belong to the same section as the option.")

    def __str__(self):
        return f"{self.section.title} → {self.label}"
    
    
class CompanyAccreditation(models.Model):
    """
    Through model for CompanyOperation.accreditations.
    Stores the optional audit date alongside each selected accreditation option.
    audit_date is only relevant when the linked CheckboxOption has requires_date=True.
    """
    operation  = models.ForeignKey(
        'CompanyOperation', on_delete=models.CASCADE,
        related_name='accreditation_entries'
    )
    option     = models.ForeignKey(
        CheckboxOption, on_delete=models.CASCADE,
        limit_choices_to={'section__key': 'accreditations', 'is_active': True},
    )
    audit_date = models.DateField(
        blank=True, null=True,
        help_text="Populated only when option.requires_date is True"
    )
    ...
    def clean(self):
        from django.core.exceptions import ValidationError
        if self.option_id and self.option.requires_date and not self.audit_date:
            raise ValidationError("This accreditation requires an audit date.")

    def __str__(self):
        date_str = f" – {self.audit_date}" if self.audit_date else ""
        return f"{self.operation.company.company_name} → {self.option.label}{date_str}"
...
class CompanyFleet(models.Model):
    ...
    average_age = models.CharField(max_length=50, blank=True, null=True)
    
    vehicle_types = models.ManyToManyField(
        CheckboxOption, blank=True, related_name='fleet_vehicle_types',
        limit_choices_to={'section__key': 'vehicle_types', 'is_active': True},
    )
    special_cargo = models.ManyToManyField(
        CheckboxOption, blank=True, related_name='fleet_special_cargo',
        limit_choices_to={'section__key': 'special_cargo', 'is_active': True},
    )
    nhvr_configurations = models.ManyToManyField(
        CheckboxOption, blank=True, related_name='fleet_nhvr_configs',
        limit_choices_to={'section__key': 'nhvr_configurations', 'is_active': True},
    )
    ...
    @property
    def nhvr_grouped(self):
        from itertools import groupby
        opts = self.nhvr_configurations.select_related('category').order_by('category__order', 'order')
        return {cat: list(items) for cat, items in groupby(opts, key=lambda o: o.category)}
...
class CompanyOperation(models.Model):
    ...
    work_types = models.ManyToManyField(
        CheckboxOption, blank=True, related_name='ops_work_types',
        limit_choices_to={'section__key': 'work_types', 'is_active': True},
    )
    accreditations = models.ManyToManyField(
        CheckboxOption, blank=True,
        through='CompanyAccreditation',
        related_name='ops_accreditations',
        limit_choices_to={'section__key': 'accreditations', 'is_active': True},
    )
    operating_areas = models.ManyToManyField(
        CheckboxOption, blank=True, related_name='ops_operating_areas',
        limit_choices_to={'section__key': 'operating_areas', 'is_active': True},
    )
...
class CompanyRiskProfile(models.Model):
    ...
    safety_policies = models.ManyToManyField(
        CheckboxOption, blank=True, related_name='risk_safety_policies',
        limit_choices_to={'section__key': 'safety_policies', 'is_active': True},
    )
...
class CompanySubcontractorProfile(models.Model):
    ...
    compliance_practices = models.ManyToManyField(
        CheckboxOption, blank=True, related_name='sub_compliance_practices',
        limit_choices_to={'section__key': 'compliance_practices', 'is_active': True},
    )
...
class CompanyIncidentProfile(models.Model):
    ...
    reporting_process = models.ManyToManyField(
        CheckboxOption, blank=True, related_name='inc_reporting_process',
        limit_choices_to={'section__key': 'reporting_process', 'is_active': True},
    )
