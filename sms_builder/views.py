...
from django.shortcuts import render
from .models import *
# from .pdf_services.pdf_generator_service import build_company_pdf
...
def sms_builder_form(request):
    context = {'company': None, 'is_edit': False}
    ...
    # ── Build selected-option lookup + accreditation audit-date map ──
    selected_ids = set()
    audit_dates = {}  # {option_id: 'YYYY-MM-DD'}
    ...
    if company:
        ...
        if op:
            selected_ids.update(op.work_types.values_list('id', flat=True))
            selected_ids.update(op.operating_areas.values_list('id', flat=True))
            for entry in CompanyAccreditation.objects.filter(operation=op).select_related('option'):
                selected_ids.add(entry.option_id)
                if entry.audit_date:
                    audit_dates[entry.option_id] = entry.audit_date.strftime('%Y-%m-%d')
        if fl:
            selected_ids.update(fl.vehicle_types.values_list('id', flat=True))
            selected_ids.update(fl.special_cargo.values_list('id', flat=True))
            selected_ids.update(fl.nhvr_configurations.values_list('id', flat=True))
        if rp:
            selected_ids.update(rp.safety_policies.values_list('id', flat=True))
        if sp:
            selected_ids.update(sp.compliance_practices.values_list('id', flat=True))
        if ip:
            selected_ids.update(ip.reporting_process.values_list('id', flat=True))

    # ── Build sections dict (keyed by section.key) ──
    sections_qs = Section.objects.select_related('tab').all()
    context['sections'] = {s.key: s for s in sections_qs}

    # ── Helper to serialize options for the template ──
    def opts(section_key, include_audit=False):
        try:
            section = context['sections'][section_key]
        except KeyError:
            return []
        out = []
        for o in section.options.filter(is_active=True).order_by('order'):
            row = {
                'id': o.id,
                'label': o.label,
                'value': o.value,
            ...
            if include_audit:
                row['requires_date'] = o.requires_date
                row['audit_date'] = audit_dates.get(o.id, '')
            out.append(row)
        return out

    context['options'] = {
        'work_types':           opts('work_types'),
        'accreditations':       opts('accreditations', include_audit=True),
        'operating_areas':      opts('operating_areas'),
        'vehicle_types':        opts('vehicle_types'),
        'special_cargo':        opts('special_cargo'),
        'safety_policies':      opts('safety_policies'),
        'compliance_practices': opts('compliance_practices'),
        'reporting_process':    opts('reporting_process'),
    }

    # ── NHVR: nested by category ──
    nhvr_section = context['sections'].get('nhvr_configurations')
    nhvr_groups = []
    if nhvr_section:
        for cat in nhvr_section.categories.filter(is_active=True).order_by('order'):
            nhvr_groups.append({
                'id': cat.id,
                'name': cat.name,
                ...
                'options': [
                    ...
                    for o in cat.options.filter(is_active=True).order_by('order')
                ],
            })
    context['nhvr_groups'] = nhvr_groups
...
def save_company_profile(request):
    ...
    try:
        ...
        # Step 1 scalars (also used for user-creation flow below)
        ...
        if not user.is_authenticated:
            ...
            login(request, user)
            # Optional: send_welcome_email(user.email, random_password)
        ...
        # ── STEP 2: OPERATIONS ──
        step2_scalars = format_step2_data(data)
        operation, _ = CompanyOperation.objects.update_or_create(
            company=company,
            defaults=step2_scalars,
        )
        # Plain M2M fields
        operation.work_types.set(data.get('work_types', []))
        operation.operating_areas.set(data.get('operating_areas', []))

        # Accreditations use a custom through model (with audit_date) — rebuild rows
        CompanyAccreditation.objects.filter(operation=operation).delete()
        for entry in data.get('accreditations', []):
            option_id = entry.get('option_id')
            if not option_id:
                continue
            audit_date = entry.get('audit_date') or None
            CompanyAccreditation.objects.create(
                operation=operation,
                option_id=option_id,
        ...
        # ── STEP 3: FLEET ──
        step3_scalars = format_step3_data(data)
        fleet, _ = CompanyFleet.objects.update_or_create(
            company=company,
            defaults=step3_scalars,
        )
        fleet.vehicle_types.set(data.get('vehicle_types', []))
        fleet.special_cargo.set(data.get('special_cargo', []))
        fleet.nhvr_configurations.set(data.get('nhvr_configurations', []))

        # ── STEP 4: RISK (STATIC) ──
        step4_static = format_step4_static_data(data)
        risk_profile, _ = CompanyRiskProfile.objects.update_or_create(
            company=company,
            defaults=step4_static,
        )
        risk_profile.safety_policies.set(data.get('safety_policies', []))

        # ── STEP 4: RISK (DYNAMIC HAZARDS) ──
        hazards_data = format_step4_dynamic_data(data)
        RiskHazard.objects.filter(company=company).delete()
        if hazards_data:
            RiskHazard.objects.bulk_create([
                RiskHazard(
                    company=company,
        ...
        # ── STEP 5: SUBCONTRACTORS (STATIC) ──
        step5_static = format_step5_static_data(data)
        sub_profile, _ = CompanySubcontractorProfile.objects.update_or_create(
            company=company,
            defaults=step5_static,
        )
        sub_profile.compliance_practices.set(data.get('compliance_practices', []))
        ...
        # ── STEP 6: INCIDENTS (STATIC) ──
        step6_static = format_step6_static_data(data)
        incident_profile, _ = CompanyIncidentProfile.objects.update_or_create(
            company=company,
            defaults=step6_static,
        )
        incident_profile.reporting_process.set(data.get('reporting_process', []))
...
def generate_company_documents(request):
    ...
    file_name = f"{safe_name}_SMS_Document.pdf"
    save_path = f"company_docs/generated/{file_name}"
    ...
    return JsonResponse({
        "status":      "success",
        "message":     "Document generated successfully.",
        "file_url":    doc_obj.file.url if doc_obj.file else "",
        "file_name":   file_name,
        "uploaded_on": doc_obj.created_at.strftime("%b %d, %Y"),
    })
